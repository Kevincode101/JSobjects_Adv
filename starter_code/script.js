//Task 1: Console the year in which the song "Beat it" was relased 
//alert(library.music.eighties.Michael_Jackson[0].year);

//Task 2: Console the name of the song Lady Gaga released in 2006 
//alert(library.music.)

  
//( document ).ready(function() {
//Task 3: Have the words "Rolling in the Deep" appear on the HTML page in a div tag.    
    
//Task 4: (project) Create a site that can display songs from an artist specified by a user search.
    var artist = document.getElementById("music").value;
    $("#submit").click(function(){
        if(artist === "adele"){
           alert(artist); 
        }
        
    });
var eighties = library.music.eighties
var nineties = library.music.nineties
var two_thousands = library.music.two_thousands
var twenty_tens = library.music.twenty_tens
